...
import Prelude.Extended --( (*),(+),(-),(/),(<=),(>>=),(==),($),(&&),(<),(<>),bind,discard,div,otherwise,void,show,pure,unit, uidx, uJust, error, fromList, range, debug )
import Math ( sqrt   )
...
doCholdc a n = a' where
  ll = n*n -- `debug` "...entering doCholdc"
  l = run (do
    ...
    ST.for 0 n \i0 -> do
      ST.for i0 n \j0 -> do
        ...
        ST.for 0 (i0+1) \k0 -> do
          ...
          let sum = (uJust maij) - (uJust maik) * (uJust majk) -- `debug` (show "i0j0k0 aik ajk aij" <> show i0 <> show j0 <> show k0 <> show maik <> show majk <> show maij )
          void $ if i0==j0
    ...
    ST.for 0 n \i0 -> do
      ...
      void $ STA.poke (ixarr i0 i0) (uJust maii) arr -- `debug` (show maii)
